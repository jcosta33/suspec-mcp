...
// A scratch workspace carrying the artifacts the stub reads (it sniffs the checked file's
...
describe('invoke_suspec — the subprocess edge', () => {
    ...
    it('refuses every verb the CLI does not have (check is the whole surface)', () => {
        for (const verb of ['status', 'store', 'show', 'review', 'new', 'write', 'work', 'done', 'evidence', 'promote']) {
            expect(() => invoke_suspec(env(stub), verb), verb).toThrow(/non-allow-listed/);
        }
    });

    it('always appends --json and never a mutation-shaped flag', () => {
        const s = scratchEnv(stub);
        try {
            const r = invoke_suspec(s.env, 'check', ['spec.md']);
    ...
    it('returns kind:"ok" with the parsed data on a check success (and relays the warnings exit code)', () => {
        const s = scratchEnv(stub);
        try {
            const r = invoke_suspec(s.env, 'check', ['spec.md']);
            ...
            if (r.kind === 'ok') {
                expect((r.data as { diagnostics: unknown[] }).diagnostics.length).toBeGreaterThan(0);
            }
        } finally {
            s.cleanup();
        }
    ...
    it('passes the allow-listed companion flags (--spec/--task) through to check', () => {
        const s = scratchEnv(stub);
        try {
            const r = invoke_suspec(s.env, 'check', ['review.md'], {
                flags: { '--spec': 'spec.md', '--task': 'task.md' },
            });
            expect(r.kind).toBe('ok');
            expect(r.invocation.command).toBe(
                'suspec check review.md --spec spec.md --task task.md --json',
            );
        } finally {
            s.cleanup();
        }
    ...
    it('passes the bare --contract flag through (the contract dump invocation)', () => {
        const r = invoke_suspec(env(stub), 'check', [], { bare: ['--contract'] });
        expect(r.kind).toBe('ok');
        expect(r.invocation.command).toBe('suspec check --contract --json');
        if (r.kind === 'ok') {
    ...
    it('refuses a non-allow-listed VALUED flag (a slip that tried --from or --write would throw)', () => {
        for (const flag of ['--write', '--launch', '--from', '--scope', '--base']) {
            expect(() =>
                invoke_suspec(env(stub), 'check', ['spec.md'], { flags: { [flag]: 'x' } }),
    ...
    it('returns kind:"launch-error" when the binary cannot be launched', () => {
        const r = invoke_suspec(env('/nonexistent/suspec-does-not-exist'), 'check', ['x.md']);
    ...
    it('returns kind:"launch-error" with the stderr tail when output is non-JSON', () => {
        const r = invoke_suspec(env(nonjson), 'check', ['garbage']);
        expect(r.kind).toBe('launch-error');
        if (r.kind === 'launch-error') {
    ...
    it('returns kind:"launch-error" when the CLI produces empty output', () => {
        const r = invoke_suspec(env(nonjson), 'check', ['empty']);
        expect(r.kind).toBe('launch-error');
        if (r.kind === 'launch-error') {
            expect(r.message).toMatch(/no parseable JSON/);
        }
    ...
    it('catches a synchronous spawn throw (NUL-byte arg) and returns a clean launch-error, never escapes', () => {
        ...
        const r = invoke_suspec(env(stub), 'check', ['bad\x00path.md']);
        expect(r.kind).toBe('launch-error');
        if (r.kind === 'launch-error') {
